...
func (c *AutoRegisterController) Run(workers int, stopCh <-chan struct{}) {
	...
	// NOTE: we only want to manage APIService objects managed by grafana-kube-aggregator piece here
	// Since the value of it's label can be either true or onstart, we simply check for existence when listing all
	// pertinent services
	serviceReq, _ := labels.NewRequirement(AutoRegisterManagedLabel, selection.Exists, nil)
	selector.Add(*serviceReq)
	if services, err := c.apiServiceLister.List(selector); err == nil {
